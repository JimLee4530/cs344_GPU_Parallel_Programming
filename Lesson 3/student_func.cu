...
__global__ void min(float *d_out, const float* const d_in)
{
...
__global__ void max(float *d_out, const float* const d_in)
{
  extern __shared__ float sdata[];
  int myId = threadIdx.x + blockDim.x * blockIdx.x;
  int tid = threadIdx.x;
...
__global__ void simple_histo(unsigned int *d_bins, const float* const d_logLuminance, const int numBins, const float min_logLum, const float logLumRange)
{
    int myId = threadIdx.x + blockDim.x * blockIdx.x;
    int myBin = (d_logLuminance[myId] - min_logLum) / logLumRange * numBins;
                                  ...
                                  const size_t numBins)
{
  ...
  //float *min_block = new float[numCols];
  const int m = 1024;
  int blocks = numCols * numRows / m;
  ...
  min<<<blocks, m, m * sizeof(float)>>>(d_intermediate, d_logLuminance);
  min<<<1, blocks, blocks * sizeof(float)>>>(d_min, d_intermediate);

  max<<<blocks, m, m * sizeof(float)>>>(d_intermediate, d_logLuminance);
  max<<<1, blocks, blocks * sizeof(float)>>>(d_max, d_intermediate);
  ...
  simple_histo<<<blocks, m>>>(d_cdf, d_logLuminance, numBins, min_logLum, logLumRange);
